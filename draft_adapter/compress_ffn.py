...
class SwiftSVDCompressor:
    ...
    def _selected_attention_indices(
        self,
        layer_idx: int,
        device: torch.device,
    ) -> tuple[Tensor, Tensor]:
        """Select complete GQA groups so Q heads keep their original KV head."""
        nh = self.arch.num_attention_heads
        nk = self.arch.num_kv_heads
        target_nh = self.arch.target_num_heads
        target_nk = self.arch.target_num_kv_heads
        groups = nh // nk
        if target_nh != target_nk * groups:
            raise ValueError(
                "Head pruning must preserve the original GQA group size: "
                f"{target_nh} Q heads != {target_nk} KV heads * {groups}"
            )

        q_scores = self.head_importance[layer_idx].reshape(nk, groups).mean(-1)
        kv_scores = self.kv_importance[layer_idx]
        q_scores = q_scores / q_scores.max().clamp_min(1e-8)
        kv_scores = kv_scores / kv_scores.max().clamp_min(1e-8)
        kv_indices = (q_scores + kv_scores).topk(target_nk).indices.sort()[0]
        offsets = torch.arange(groups, device=kv_indices.device)
        q_indices = (kv_indices[:, None] * groups + offsets).reshape(-1)
        return q_indices.to(device), kv_indices.to(device)
            ...
            if "self_attn" in key:
                ...
                if "q_norm.weight" in key:
                    if src[key].numel() == hd:
                        ...
                        w = src[key].reshape(nh, hd)[q_idx].reshape(-1)
                        dst[key].copy_(w)
                    continue
                ...
                if "k_norm.weight" in key:
                    if src[key].numel() == hd:
                        ...
                        w = src[key].reshape(nk, hd)[kv_idx].reshape(-1)
                        dst[key].copy_(w)
                    continue

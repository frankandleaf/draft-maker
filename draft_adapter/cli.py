...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--method", default="slicegpt",
                   ...
                   help="Compression algorithm: slicegpt (PCA) or swift-svd (FFN+heads)")
...
def run_pipeline(config: PipelineConfig) -> None:
    ...
    # Swift-SVD: only compress FFN + attention heads, hidden_size stays
    if config.method == "swift-svd":
        saved_es = config.width.embed_size_factor
        saved_hs = config.width.head_size_factor
        config.width.embed_size_factor = 1.0   # hidden_size untouched
        config.width.head_size_factor = 1.0     # num_heads computed below
        arch = compute_targets(arch, config.width, config.depth)
        # Override: only intermediate_size and num_heads change
        arch.target_embed_dim = arch.hidden_size
        arch.target_head_dim = arch.head_dim   # frozen
        arch.target_intermediate_size = max(8, int(arch.intermediate_size * saved_es))
        arch.target_num_heads = max(1, int(arch.num_attention_heads * saved_hs))
        kv_groups = arch.num_kv_groups
        arch.target_num_kv_heads = max(1, arch.target_num_heads // kv_groups)
    else:
        arch = compute_targets(arch, config.width, config.depth)
    ...
    # Use device_map="auto" for multi-GPU (teacher splits across GPUs),
    # device_map="cuda" for single-GPU.
    device_map = "auto" if config.device == "cuda" else config.device
    ...
    # ============================================================
    if config.method == "swift-svd":
        # Swift-SVD: activation-aware FFN + head pruning, no PCA, no rotation

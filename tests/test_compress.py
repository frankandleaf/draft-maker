...
class TestProjectionRules:
    ...
    def test_q_proj_is_both(self):
        assert _get_projection_rule("model.layers.0.self_attn.q_proj.weight") == "both"
    ...
    def test_o_proj_is_both(self):
        assert _get_projection_rule("model.layers.0.self_attn.o_proj.weight") == "both"
...
class TestWidthCompressorProjection:
    ...
    def test_q_proj_both_sides(self, compressor):
        """q_proj [d,d] → both-side projection → [d',d']."""
        d, d_prime = 1024, 512
        W_q = torch.randn(d, d)  # [out=num_heads*head_dim, in=hidden_size]
        result = compressor._project_weight(W_q, "model.layers.0.self_attn.q_proj.weight")
        assert result.shape == (d_prime, d_prime)

    def test_o_proj_both_sides(self, compressor):
        """o_proj [d,d] → both-side projection → [d',d']."""
        d, d_prime = 1024, 512
        W_o = torch.randn(d, d)
